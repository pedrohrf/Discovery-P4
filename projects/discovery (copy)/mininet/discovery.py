...
class DiscoverySwitch(Topo):
    ...
    def add_switches(self, sw_path, json_path, thrift_port, pcap_dump, debug, **opts):
        self.networkSwitches.extend((
            self.addSwitch('s1', sw_path = sw_path, json_path = 'build/discovery.json', thrift_port = 9090, pcap_dump = pcap_dump, enable_debugger = debug),
            self.addSwitch('s2', sw_path = sw_path, json_path = 'build/discovery.json', thrift_port = 9091, pcap_dump = pcap_dump, enable_debugger = debug),
            self.addSwitch('s3', sw_path = sw_path, json_path = 'build/discovery.json', thrift_port = 9092, pcap_dump = pcap_dump, enable_debugger = debug),
            self.addSwitch('s4', sw_path = sw_path, json_path = 'build/discovery.json', thrift_port = 9093, pcap_dump = pcap_dump, enable_debugger = debug)
        ))
    ...
    def build_network_topo(self):
        ...
        self.addLink(self.networkSwitches[0], self.networkHosts[4])
        print('s1 <-> h5:')
        print(self.port(self.networkSwitches[0], self.networkHosts[4]))

        self.addLink(self.networkSwitches[1], self.networkHosts[4])
        print('s2 <-> h5:')
        print(self.port(self.networkSwitches[1], self.networkHosts[4]))

        self.addLink(self.networkSwitches[2], self.networkHosts[4])
        print('s3 <-> h5:')
        print(self.port(self.networkSwitches[2], self.networkHosts[4]))

        self.addLink(self.networkSwitches[3], self.networkHosts[4])
        print('s4 <-> h5:')
        print(self.port(self.networkSwitches[3], self.networkHosts[4]))
...
def initialize_routes(network):
    ...
    set_node_route(network.get('h5'), '10.0.4.1', '00:04:00:00:00:04', 'dev eth0 via 10.0.4.1')
